...
function buildApp(appPath, buildLocation, buildOptions) {
  return new Promise((resolve, reject) => {
    ...
    var outputDir = (buildOptions.outputDir ? path.resolve(buildOptions.outputDir, uuid.v4()) : buildLocaltion)
  
    buildMeteorApp(appPath, outputDir, buildOptions, function(code) {
      if (code === 0) {
        archiveIt(outputDir, callback);
      } else {
...
function buildMeteorApp(appPath, buildLocation, buildOptions, callback) {
 var executable = buildOptions.executable || 'meteor';
  var server = buildOptions.server || 'http://localhost:3000'
  var args = [
